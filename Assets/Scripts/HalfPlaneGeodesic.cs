...
public class HalfPlaneGeodesic : Geodesic
{
    public override void DrawGeodesic()
    {
        ...
        Debug.Log("hp gen " + modelView);

        Vector2 center = HalfPlaneModel.HalfPlaneArcCenterFromLine(p1, p2);
        radius = HalfPlaneModel.HalfPlaneArcRadiusFromCenter(center, p2);
        Vector2 angles = PoincareDiscModel.PoincareArcAngles(p1, p2, center);
        Debug.Log("hp gen " + center + "," + radius +","+ angles);
